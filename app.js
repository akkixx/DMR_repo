class DMRApp {
    constructor() {
        ...
        window.isAndroid = window.location.protocol === 'file:';
        window.baseUrl = window.isAndroid ? 'file:///android_asset/public/' : './';
    ...
    checkAuth() {
        ...
        if (!user || !user.isAuthenticated) {
            window.location.href = window.baseUrl + 'auth.html';
            return false;
        }
        return true;
    }
    ...
    showNotification(medName, isReminder = false) {
        ...
        const notifContainer = document.querySelector('.notification-container') || this.createNotificationContainer();
        
        const notif = document.createElement('div');
        notif.className = 'notification';
        notif.innerHTML = `
            <div class="notification-icon">${isReminder ? '⏰' : '💊'}</div>
            <div class="notification-content">
                <div class="notification-title">${isReminder ? 'Reminder' : 'Medication Due'}</div>
                <div class="notification-text">${medName}</div>
            </div>
        ...
        if (MedicationData.currentUser.settings.sound) {
            try {
                // Create absolute path based on where we're running
                const audioSrc = window.location.protocol === 'file:' 
                    ? 'file:///android_asset/public/assets/notification.mp3' 
    ...
    logout() {
        ...
        document.querySelector('.app-container').classList.add('logout-transition');
